...
for example_src in examples/*/*.cpp examples/*.cpp; do
    [ -e "$example_src" ] || continue

echo "==============================================="
echo "Building and running example: $example_src"
echo "==============================================="

OUT_BIN="build/$(basename "${example_src%.cpp}")"
mkdir -p "$(dirname "$OUT_BIN")"

if [ "$ON_APPLE" = "true" ]; then
    # Build command for Apple (macOS).
    $CXX $CXXFLAGS $INCLUDE_FLAGS $LIB_CPP_SOURCES "$example_src" \
