...
static utils::Ref<ir::Tensor> rand_tensor(const std::vector<size_t> &shape, float scale, uint32_t &seed,
    ...
    for (auto &f : v) {
        ...
        f = (((seed >> 8) / float(1u << 24)) * 2.f - 1.f) * scale;
    }
    return ir::from_vector<float>(v, shape, dev);
}
static std::vector<int32_t> gen(Qwen3Model &m, llm::PrefixCacheSession &s, const std::vector<int32_t> &prompt, int N) {
...
int main() {
    try {
        ...
        cfg.layer_types = {LayerType::LINEAR_ATTENTION, LayerType::LINEAR_ATTENTION, LayerType::LINEAR_ATTENTION,
        ...
        std::string path =
            std::string(std::getenv("TMPDIR") ? std::getenv("TMPDIR") : "/tmp") + "/cppgrad_prefix_cache_test.bin";

        // Session A: prefill prompt1 (populates cache), then SAVE to disk.
        llm::PrefixCacheSession a;
        a.capacity_hint = 64;
