...
static float run_case(backend::DeviceType dev, size_t L, size_t H, size_t D, std::vector<int> block_sizes) {
    ...
    for (int S : block_sizes) {
        ...
        for (size_t i = 0; i < rows * cols; ++i)
            worst = std::max(worst, std::abs(pg[i] - ref[i]));
        pos += S;
    }
    return worst;
}
...
static float run_case_attn(backend::DeviceType dev, size_t H, size_t D, std::vector<int> block_sizes) {
    ...
    for (int s : block_sizes)
        total += s;
    auto kc = ir::parameter({1, total, H, D}, dev, common::DType::FLOAT32, true);
    kc->set_requires_grad(false);
    auto vc = ir::parameter({1, total, H, D}, dev, common::DType::FLOAT32, true);
    vc->set_requires_grad(false);

    std::vector<float> khist, vhist; // full K/V history (for the fresh-cache comparison)
    size_t pos = 0;
    float fill = 1.0f, worst = 0.0f;
    ...
    for (int S : block_sizes) {
        ...
        auto kc2 = ir::parameter({1, end, H, D}, dev, common::DType::FLOAT32, true);
        kc2->set_requires_grad(false);
        auto vc2 = ir::parameter({1, end, H, D}, dev, common::DType::FLOAT32, true);
        vc2->set_requires_grad(false);
        ...
        for (size_t i = 0; i < inc.size(); ++i)
            worst = std::max(worst, std::abs(inc[i] - fresh[i]));
        pos += S;
    }
    return worst;
}
...
int main() {
    ...
    for (auto dev : devs) {
        ...
        printf("[%s] cache_update S=1:%g S=4:%g mixed:%g | SDPA incr-vs-fresh  S=1:%g  S=4:%g\n", name, d1, dN, dM, a1,
               aN);
        if (d1 > 1e-6f || dN > 1e-6f || dM > 1e-6f || a1 > 1e-4f || aN > 1e-4f)
            ok = false;
    }
