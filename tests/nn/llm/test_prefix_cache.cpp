...
static std::vector<int32_t> gen(Qwen3Model &m, llm::PrefixCacheSession &sess, const std::vector<int32_t> &prompt,
                                int N) {
    std::unordered_set<int32_t> stop; // empty: produce exactly N tokens
    return llm::generate_with_prefix_cache(
        m, sess, prompt, N, [](int32_t) { return true; }, stop, llm::SamplingParams{});
}
...
int main() {
    try {
        ...
        std::cout << "  turn2 prompt len=" << prompt2.size() << "  reused=" << reused << " prefilled=" << prefilled
