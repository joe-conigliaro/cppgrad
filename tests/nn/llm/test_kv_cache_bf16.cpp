...
static bool attn(backend::DeviceType dev, size_t S, size_t KV, size_t nKV, size_t n_rep, size_t Dh) {
    ...
    auto fill = [&](common::DType dt, const utils::Ref<ir::Tensor> &src) {
        auto c = ir::parameter({1, KV, nKV, Dh}, dev, dt, true);
    ...
    auto ref_gqa =
        nn::functional::gqa_attention(q, kf32, vf32, nullptr, n_rep)->to_vector<float>(); // non-causal ok: same cache
    auto got_gqa = nn::functional::gqa_attention(q, kbf, vbf, nullptr, n_rep)->to_vector<float>();

    float wf = 0, wg = 0;
    for (size_t i = 0; i < ref_flash.size(); ++i)
        wf = std::max(wf, std::fabs(ref_flash[i] - got_flash[i]));
    for (size_t i = 0; i < ref_gqa.size(); ++i)
        wg = std::max(wg, std::fabs(ref_gqa[i] - got_gqa[i]));
...
int main() {
    ...
    if (metal)
        ok &= roundtrip(backend::DeviceType::METAL);
