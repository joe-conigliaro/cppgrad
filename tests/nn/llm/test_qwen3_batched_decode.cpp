...
static bool check(const std::string& name, const Qwen3Config& cfg, backend::DeviceType device) {
    ...
    std::vector<int32_t> seqs(B * N);  // [B, N] generated tokens
    {
        ...
        for (size_t b = 0; b < B; ++b) { cur[b] = model.greedy_at(logits, b, P - 1); seqs[b * N + 0] = cur[b]; }
        size_t pos = P;
        for (size_t k = 1; k < N; ++k) {
            auto lg = model.forward_cached_batched(cur, B, 1, pos, caches);   // [B,1,V]
            for (size_t b = 0; b < B; ++b) { cur[b] = model.greedy_at(lg, b, 0); seqs[b * N + k] = cur[b]; }
            ++pos;
        }
    }
    ...
    for (size_t b = 0; b < B; ++b) {
        for (size_t i = 0; i < P; ++i) full[b * (P + N) + i] = prompts[b * P + i];
        for (size_t k = 0; k < N; ++k) full[b * (P + N) + P + k] = seqs[b * N + k];
    }
    bool ok = true;
    {
        ir::NoGradScope ng;
...
int main() {
    ...
    Qwen3Config full{
        64, 2, 128, 256, 4096, 4, 2, 16, 0, 0, 0, 0, 0,
        1e-6, 1000000, false, {}, 1.0, false, "", {}, 0,
    };
    Qwen3Config mixed{
        64, 4, 128, 256, 4096, 4, 2, 16, 16, 2, 16, 4, 4,
        1e-6, 1000000, false, {}, 1.0, true, "swish",
        {LayerType::LINEAR_ATTENTION, LayerType::LINEAR_ATTENTION,
         LayerType::LINEAR_ATTENTION, LayerType::FULL_ATTENTION}, 4,
    };
