...
int main() {
    ...
    for (auto& v : q) v = (uint8_t)(rnd() * 256.f);          // codes in [0,255]
    ...
    backend::cpu::matmul_quant_affine_f32(A.data(), qw.data(), scales.data(), biases.data(),
                                          out.data(), M, N, K, gs);

    double max_abs = 0.0;
    for (size_t i = 0; i < M * N; ++i) max_abs = std::max(max_abs, std::fabs((double)out[i] - ref[i]));
