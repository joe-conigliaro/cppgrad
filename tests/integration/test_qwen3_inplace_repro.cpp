...
static void quantize_linear(Linear& ql, backend::DeviceType dev) {
    ...
    if (in % gs != 0) throw std::runtime_error("quantize_linear: in dim must be multiple of 64");
    ...
    for (size_t n = 0; n < out; ++n)
        for (size_t g = 0; g < G; ++g) {
            ...
            for (size_t j = 0; j < (size_t)gs; ++j) { float val = w[(g*gs+j)*out + n]; mn = std::min(mn, val); mx = std::max(mx, val); }
            float scale = (mx - mn) / 255.0f; if (scale < 1e-12f) scale = 1e-12f;
            sc[n*G+g] = scale; bs[n*G+g] = mn;
            for (size_t j = 0; j < (size_t)gs; ++j) {
                size_t k = g*gs + j; float val = w[k*out + n];
                int code = (int)std::lround((val - mn) / scale); code = std::max(0, std::min(255, code));
...
int main() {
    try {
        ...
        for (int L : {4, 8}) {
            ...
            model.inplace_kv = false; auto ref = model.generate(prompt, N);
            model.inplace_kv = true;  auto inp = model.generate(prompt, N);

            bool match = (ref == inp);
            size_t first_diff = 0; while (first_diff < ref.size() && first_diff < inp.size() && ref[first_diff] == inp[first_diff]) ++first_diff;
            std::cout << "L=" << L << ": " << (match ? "MATCH" : "DIFFER")
                      << (match ? "" : " (first diff at token " + std::to_string(first_diff) + ")") << "\n";
            if (!match) {
                std::cout << "  concat   :"; for (int t : ref) std::cout << " " << t; std::cout << "\n";
                std::cout << "  in-place :"; for (int t : inp) std::cout << " " << t; std::cout << "\n";
            }
        }
