...
int main() {
    ...
    if (!dir) {
        printf("[skip] set QWEN_MTP_DIR to run the MTP validation test\n");
        return 0;
    }
    ...
    Qwen3Model model(config == "27b" ? Qwen3Config::get_27b() : Qwen3Config::get_27b(), device, /*lazy_weights=*/true);
    ...
    auto spec = model.generateSpeculativeMTP(
        prompt, N, [](int32_t) { return true; }, eos, /*n_draft=*/4, cppgrad::nn::llm::qwen::SamplingParams{}, &stats);
    ...
    printf("vs greedy: %s\n", first_diff < 0
                                  ? "identical"
                                  : ("first diff at token " + std::to_string(first_diff) + " (fp near-tie)").c_str());
