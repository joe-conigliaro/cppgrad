...
static std::vector<float> vec(size_t n){ std::vector<float> v(n); for(auto&x:v)x=rnd(); return v; }
...
static std::vector<float> ref_bmm(const std::vector<float>& A, const std::vector<size_t>& as,
    ...
    std::vector<size_t> ob(nb); size_t bc=1;
    for(size_t i=0;i<nb;++i){ size_t ia=i<pa?1:ab[i-pa], ib=i<pb?1:bb[i-pb]; ob[i]=std::max(ia,ib); bc*=ob[i]; }
    ...
    for(size_t bi=0;bi<bc;++bi){
        ...
        for(size_t d=nb;d-->0;){ idx[d]=rem%ob[d]; rem/=ob[d]; }
        for(size_t d=0;d<ab.size();++d){ astr*=1; }
        // compute strides
        size_t as_=1; for(size_t d=ab.size();d-->0;){ size_t od=d+pa; aoff += (ab[d]==1?0:idx[od])*as_; as_*=ab[d]; }
        size_t bs_=1; for(size_t d=bb.size();d-->0;){ size_t od=d+pb; boff += (bb[d]==1?0:idx[od])*bs_; bs_*=bb[d]; }
        const float* Ap=A.data()+aoff*M*K; const float* Bp=B.data()+boff*K*N;
        for(size_t m=0;m<M;++m)for(size_t n=0;n<N;++n){ float acc=0; for(size_t k=0;k<K;++k) acc+=Ap[m*K+k]*Bp[k*N+n]; out[(bi*M+m)*N+n]=acc; }
    }
    return out;
}
...
static bool one(backend::DeviceType dev, std::vector<size_t> as, std::vector<size_t> bs, const char* tag){
    ir::NoGradScope ng;
    size_t an=1; for(auto d:as)an*=d; size_t bn=1; for(auto d:bs)bn*=d;
    ...
    float w=0; for(size_t i=0;i<ref.size() && i<out.size();++i) w=std::max(w,std::fabs(ref[i]-out[i]));
