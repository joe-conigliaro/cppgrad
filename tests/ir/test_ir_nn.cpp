...
static void test_broadcast_backward_add_mean() {
    ...
    for (float v : gX)
        EXPECT_CLOSE(v, 1.0f / 12.0f, 1e-7f, "grad X val");
    for (float v : gb)
        EXPECT_CLOSE(v, 1.0f / 3.0f, 1e-7f, "grad b val");
...
static void test_linear_bias_grad_mean() {
    ...
    for (float v : gb)
        EXPECT_CLOSE(v, 1.0f / (float)O, 1e-7f, "grad b = 1/O");
...
static void test_tanh_backward() {
    ...
    auto x = ir::from_vector(std::vector<float>{-1.0f, 0.0f, 1.0f}, {3});
...
static void test_relu_backward() {
    ...
    auto x = ir::from_vector(std::vector<float>{-1.0f, 0.0f, 2.0f}, {3});
    x->set_requires_grad(true);
    auto y = ir::relu(x);
    auto L = ir::mean(y);
    L->backward();
...
static void test_mse_loss_perfect_fit() {
    ...
    for (float v : gyhat)
        EXPECT_CLOSE(v, 0.0f, 1e-7f, "mse grad at perfect fit");
...
static void test_softmax_cross_entropy_with_logits_basic() {
    ...
    auto logits = ir::from_vector(std::vector<float>{2.0f, 0.0f, -1.0f, -1.0f, 3.0f, 0.0f}, {2, 3});
    logits->set_requires_grad(true);

    auto targets = ir::from_vector(
        std::vector<float>{
