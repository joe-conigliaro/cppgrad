...
static void test_backward_add_mean_explicit_vs_builtin() {
    ...
    // print the first row
    std::cout << "gX1[0..5]: ";
    for (int i = 0; i < std::min<int>(6, gX1.size()); ++i)
        std::cout << gX1[i] << (i + 1 < 6 ? ", " : "\n");
    std::cout << "gX2[0..5]: ";
    for (int i = 0; i < std::min<int>(6, gX2.size()); ++i)
        std::cout << gX2[i] << (i + 1 < 6 ? ", " : "\n");
}
...
static void test_add_backward_with_manual_upstream() {
    ...
    for (float v : vX)
        EXPECT_CLOSE(v, 1.0f / 12.0f, 1e-6f, "dX element");
    for (float v : vb)
        EXPECT_CLOSE(v, 1.0f / 3.0f, 1e-6f, "db element");
}
...
static void test_mean_backward_no_broadcast() {
    ...
    for (float e : v)
        EXPECT_CLOSE(e, 1.0f / 12.0f, 1e-6f, "each grad 1/12");
}
