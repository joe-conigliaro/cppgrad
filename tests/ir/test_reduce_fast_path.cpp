...
static void test_reduce_last_rank2_small_inner() {
    ...
    // SUM
    {
        ...
        EXPECT_TRUE(ir::sum(X, std::vector<int>{1}, true)->shape() == std::vector<size_t>({2, 1}),
                    "sum keep_dims shape [2,1]");
    }
    // MAX
    {
        ...
        EXPECT_TRUE(ir::max(X, std::vector<int>{1}, true)->shape() == std::vector<size_t>({2, 1}),
                    "max keep_dims shape [2,1]");
    }
    // MEAN
    {
        ...
        EXPECT_TRUE(ir::mean(X, std::vector<int>{1}, true)->shape() == std::vector<size_t>({2, 1}),
                    "mean keep_dims shape [2,1]");
    }
...
static void test_reduce_last_rank2_large_inner() {
    ...
    for (int b = 0; b < B; ++b)
        v_ref[b] = v_sum[b] / static_cast<float>(N);
