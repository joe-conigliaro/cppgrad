...
#define EXPECT_CLOSE(val, exp, eps, msg)                                                                               \
    do {                                                                                                               \
        if (std::fabs((val) - (exp)) > (eps)) {                                                                        \
            std::cerr << "[FAIL] " << msg << " got=" << (val) << " expected=" << (exp) << " tol=" << (eps) << " (at "  \
                      << __FILE__ << ":" << __LINE__ << ")\n";                                                         \
            g_fail_count++;                                                                                            \
        }                                                                                                              \
    } while (0)
...
inline bool allclose(const float *a, const float *b, size_t n, const Tolerances &tol, size_t *bad_idx = nullptr,
                     float *got = nullptr, float *exp = nullptr, float *abs_err = nullptr) {
...
inline void print_mismatch(const std::string &tag, size_t idx, float got, float exp, float abs_err,
                           const Tolerances &tol) {
    std::cerr << "[FAIL] " << tag << " idx=" << idx << " got=" << got << " expected=" << exp
...
#define EXPECT_ALLCLOSE(ptr_a, ptr_b, n, tol, tag)                                                                     \
    do {                                                                                                               \
        size_t _bad = 0;                                                                                               \
        float _got = 0, _exp = 0, _err = 0;                                                                            \
