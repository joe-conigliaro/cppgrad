...
int main() {
    try {
        ...
        // data_span
        {
            std::vector<float> v{1.f, 2.f, 3.5f};
            auto t = ir::from_vector<float>(v, {3});
            auto t_cpu = (t->device_type() != backend::DeviceType::CPU) ? t->to(backend::DeviceType::CPU) : t;
        ...
        // Zero-length
        {
            ...
            auto sz_cpu = (tz->device_type() != backend::DeviceType::CPU) ? tz->to(backend::DeviceType::CPU) : tz;
