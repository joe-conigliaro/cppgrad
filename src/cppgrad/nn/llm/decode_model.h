...
class DecodeModel {
  public:
    ...
    virtual utils::Ref<ir::Tensor> prefill(const std::vector<int32_t> &tokens, size_t start_pos, ModelCache &cache) = 0;
    ...
    virtual std::unique_ptr<ModelCache> read_cache(std::istream &is, size_t valid_len, size_t capacity) = 0;
};
...
inline std::vector<int32_t> generate_with_prefix_cache(DecodeModel &model, PrefixCacheSession &sess,
    ...
    if (!prompt.empty())
        reuse = std::min(reuse, prompt.size() - 1);
...
inline bool save_prefix_cache(DecodeModel &model, const PrefixCacheSession &sess, const std::string &path) {
    if (!sess.cache || sess.tokens.empty())
        return false;
    ...
    os.write(reinterpret_cast<const char *>(sess.tokens.data()), (std::streamsize)(vlen * sizeof(int32_t)));
...
inline bool load_prefix_cache(DecodeModel &model, PrefixCacheSession &sess, const std::string &path) {
    ...
    if (!is || magic != kPrefixCacheMagic || tag != model.cache_tag())
        return false;
