...
class Qwen3Block : public Module {
  public:
    ...
    Qwen3Block(LayerType layer_type, const Qwen3Config &config,
    ...
    utils::Ref<ir::Tensor> forward(const utils::Ref<ir::Tensor> &x, const utils::Ref<ir::Tensor> &positions,
                                   const utils::Ref<ir::Tensor> &inv_freq, const utils::Ref<ir::Tensor> &mask,
    ...
    utils::Ref<ir::Tensor> forward_linear_cached(const utils::Ref<ir::Tensor> &x,
    ...
    utils::Ref<ir::Tensor> forward_full_cached(const utils::Ref<ir::Tensor> &x, const utils::Ref<ir::Tensor> &positions,
                                               const utils::Ref<ir::Tensor> &inv_freq,
                                               const utils::Ref<ir::Tensor> &mask,
                                               const utils::Ref<ir::Tensor> &k_cache,
                                               const utils::Ref<ir::Tensor> &v_cache, size_t start_pos) {
    ...
    utils::Ref<ir::Tensor>
    forward_full_cached_concat(const utils::Ref<ir::Tensor> &x, const utils::Ref<ir::Tensor> &positions,
                               const utils::Ref<ir::Tensor> &inv_freq, const utils::Ref<ir::Tensor> &mask,
                               const utils::Ref<ir::Tensor> &past_k, const utils::Ref<ir::Tensor> &past_v,
                               utils::Ref<ir::Tensor> &k_out, utils::Ref<ir::Tensor> &v_out) {
  ...
  private:
    ...
    void init_full_attention(backend::DeviceType device_type) {
        ...
        // -- FFN (SwiGLU) --
        fa_ffn = std::make_shared<GatedFFN>((size_t)H, (size_t)I, (size_t)H, GatedFFN::InnerAct::SILU, Init::Default,
                                            device_type, _lazy);
    ...
    void init_linear_attention(backend::DeviceType device_type) {
        ...
        la_ffn = std::make_shared<GatedFFN>((size_t)H, (size_t)I, (size_t)H, GatedFFN::InnerAct::SILU, Init::Default,
                                            device_type, _lazy);
    ...
    utils::Ref<ir::Tensor>
    forward_full_attention(const utils::Ref<ir::Tensor> &x, const utils::Ref<ir::Tensor> &positions,
                           const utils::Ref<ir::Tensor> &inv_freq, const utils::Ref<ir::Tensor> &mask,
                           const utils::Ref<ir::Tensor> &k_cache = nullptr, // in-place mode: preallocated cache leaf
                           const utils::Ref<ir::Tensor> &v_cache = nullptr, size_t start_pos = 0,
        ...
        q = nn::functional::apply_mrope(q, positions, inv_freq, static_cast<float>(am.partial_rotary_factor),
                                        am.mrope_interleaved);
        k = nn::functional::apply_mrope(k, positions, inv_freq, static_cast<float>(am.partial_rotary_factor),
                                        am.mrope_interleaved);
    ...
    utils::Ref<ir::Tensor> forward_linear_attention(const utils::Ref<ir::Tensor> &x,
        ...
        utils::Ref<ir::Tensor> state_out;
        auto o_bh = nn::functional::gated_delta_scan_chunked(q_bh, k_bh, v_bh, decay_bh, beta_bh, state_in, state_out,
                                                             SCAN_CHUNK); // [BH,S,val_head]
