...
enum class LayerType { FULL_ATTENTION, LINEAR_ATTENTION };

inline std::string to_string(LayerType t) {
    switch (t) {
    case LayerType::FULL_ATTENTION:
        return "full_attention";
    case LayerType::LINEAR_ATTENTION:
...
struct Qwen3Config {
    ...
    static Qwen3Config get_1_5b() {
        auto c = Qwen3Config{1536, 28, 8960, 151936,  131072, 12, 2,   128,   0,  0,  0,
                             0,    0,  1e-6, 1000000, false,  {}, 1.0, false, "", {}, 0};
        return c;
    }

    static Qwen3Config get_3b() {
        auto c = Qwen3Config{2048, 36, 10752, 151936,  131072, 16, 2,   128,   0,  0,  0,
                             0,    0,  1e-6,  1000000, false,  {}, 1.0, false, "", {}, 0};
        return c;
    }

    static Qwen3Config get_4b() {
        auto c = Qwen3Config{3072, 16, 16384, 151936,  131072, 16, 2,   192,   0,  0,  0,
                             0,    0,  1e-6,  1000000, false,  {}, 1.0, false, "", {}, 0};
        return c;
    }

    static Qwen3Config get_7b() {
        auto c = Qwen3Config{3584, 28, 18432, 151936,  131072, 28, 4,   128,   0,  0,  0,
                             0,    0,  1e-6,  1000000, false,  {}, 1.0, false, "", {}, 0};
        return c;
    }
    ...
    static Qwen3Config get_27b() {
        ...
        for (int i = 0; i < 64; ++i) {
            if (i % 4 == 3)
                layer_types.push_back(LayerType::FULL_ATTENTION);
            else
                layer_types.push_back(LayerType::LINEAR_ATTENTION);
        }
        return Qwen3Config{
            5120,        64,
            17408,       248320,
            262144, // hidden, layers, intermediate, vocab, max_pos
            24,          4,
            256, // attn_heads, kv_heads, head_dim
            128,         16,
            128,         48,
            4,                     // linear: key_dim, key_heads, val_dim, val_heads, conv_kernel
            1e-6,        10000000, // rms_norm_eps, rope_theta
            true,        {11, 11, 10},
            0.25,                 // mrope_interleaved, section, partial_rotary_factor
