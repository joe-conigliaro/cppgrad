...
inline std::tuple<float, float, bool, float> limits_for_init(Init init, size_t fan_in, size_t fan_out) {
...
class Linear : public Module {
  public:
    ...
    Linear(size_t in_features, size_t out_features, bool use_bias = true, Init init = Init::Default,
           backend::DeviceType device_type = backend::DeviceManager::default_device_type(), bool lazy = false)
        : _use_bias(use_bias) {
        if (lazy) {
            ...
            if (use_bias)
                bias = ir::parameter({1, out_features}, device_type, common::DType::FLOAT32, false);
        } else {
            auto [w_min, w_max, use_uniform, stddev] = detail::limits_for_init(init, in_features, out_features);
            auto w_init = use_uniform ? ir::uniform({in_features, out_features}, w_min, w_max, device_type)
    ...
    utils::Ref<ir::Tensor> forward(const utils::Ref<ir::Tensor> &input) override {
        if (quantized)
            return ir::quantized_matmul(input, qweight, {scales, biases}, params);
        auto output = ir::matmul(input, weight);
        if (bias)
            output = ir::add(output, bias);
        return output;
    }
