...
MLP::MLP(int in_features, int hidden_size, int out_features) {
    ...
    register_modules({std::make_shared<Linear>(in_features, hidden_size), std::make_shared<ReLU>(),
                      std::make_shared<Linear>(hidden_size, hidden_size), std::make_shared<ReLU>(),
