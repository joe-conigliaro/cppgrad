...
utils::Ref<Tensor> uniform(const std::vector<size_t>& shape, float min, float max, backend::DeviceType device_type, backend::DType dtype) {
    return Tensor::make(RandomOp{RandomOpType::UNIFORM, UniformParams{min, max}}, {}, shape, device_type, dtype);
}

utils::Ref<Tensor> normal(const std::vector<size_t>& shape, float mean, float stddev, backend::DeviceType device_type, backend::DType dtype) {
    return Tensor::make(RandomOp{RandomOpType::NORMAL, NormalParams{mean, stddev}}, {}, shape, device_type, dtype);
}

utils::Ref<Tensor> zeros(const std::vector<size_t>& shape, backend::DeviceType device_type, backend::DType dtype) {
    return full(shape, 0.0, device_type, dtype);
}

utils::Ref<Tensor> ones(const std::vector<size_t>& shape, backend::DeviceType device_type, backend::DType dtype) {
    return full(shape, 1.0, device_type, dtype);
}

utils::Ref<Tensor> full(const std::vector<size_t>& shape, double fill_value, backend::DeviceType device_type, backend::DType dtype) {
...
utils::Ref<Tensor> scalar(double value, backend::DeviceType device_type, backend::DType dtype) {
    auto t = Tensor::make(ConstantOp{ConstantOpType::SCALAR, value}, {}, std::vector<size_t>{}, device_type, dtype);
    return t;
}
