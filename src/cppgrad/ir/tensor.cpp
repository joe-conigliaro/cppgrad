...
static utils::Ref<Tensor> broadcast_to_shape(utils::Ref<Tensor> g, const std::vector<size_t> &target) {
...
static utils::Ref<Tensor> unify_to_shape(utils::Ref<Tensor> g, const std::vector<size_t> &parent_shape) {
    if (!g)
        return g;
    const auto &gs = g->shape();
    if (gs == parent_shape)
...
void Tensor::backward() {
    ...
    for (auto it = tape.rbegin(); it != tape.rend(); ++it) {
            ...
            [&](auto &&op) {
                ...
                if constexpr (std::is_same_v<T, UnaryOp>) {
                    ...
                    case UnaryOpType::RELU: {
                        auto zero = Tensor::make(ConstantOp{ConstantOpType::FULL, 0.0}, {}, x->shape(),
                                                 x->device_type(), x->dtype());
                        auto mask = Tensor::make(BinaryOp{BinaryOpType::CMP_GT}, {utils::Ref<const Tensor>(node), zero},
                                                 node->shape(), node->device_type(), node->dtype());
                        auto g = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, mask}, node->shape(),
                                              node->device_type(), node->dtype());
                        parent_grads = {g};
                        break;
                    }
                    case UnaryOpType::EXP: {
                        auto g = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, utils::Ref<const Tensor>(node)},
                                              node->shape(), node->device_type(), node->dtype());
                        parent_grads = {g};
                        break;
                    }
                    case UnaryOpType::LOG: {
                        auto g = Tensor::make(BinaryOp{BinaryOpType::DIV}, {grad_this, x}, x->shape(), x->device_type(),
                                              x->dtype());
                        parent_grads = {g};
                        break;
                    }
                    case UnaryOpType::NEG: {
                        auto g = Tensor::make(UnaryOp{UnaryOpType::NEG}, {grad_this}, grad_this->shape(),
                                              grad_this->device_type(), grad_this->dtype());
                        parent_grads = {g};
                        break;
                    }
                    case UnaryOpType::TANH: {
                        auto one = Tensor::make(ConstantOp{ConstantOpType::FULL, 1.0}, {}, node->shape(),
                                                node->device_type(), node->dtype());
                        auto out2 = Tensor::make(BinaryOp{BinaryOpType::MUL},
                                                 {utils::Ref<const Tensor>(node), utils::Ref<const Tensor>(node)},
                                                 node->shape(), node->device_type(), node->dtype());
                        auto local = Tensor::make(BinaryOp{BinaryOpType::SUB}, {one, out2}, node->shape(),
                                                  node->device_type(), node->dtype());
                        auto g = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, local}, node->shape(),
                                              node->device_type(), node->dtype());
                        parent_grads = {g};
                        break;
                    }
                    case UnaryOpType::SIN: {
                        ...
                        auto cx = Tensor::make(UnaryOp{UnaryOpType::COS}, {utils::Ref<const Tensor>(node)},
                                               node->shape(), node->device_type(), node->dtype());
                        auto g = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, cx}, node->shape(),
                                              node->device_type(), node->dtype());
                        parent_grads = {g};
                        break;
                    }
                    case UnaryOpType::COS: {
                        ...
                        auto sx = Tensor::make(UnaryOp{UnaryOpType::SIN}, {utils::Ref<const Tensor>(node)},
                                               node->shape(), node->device_type(), node->dtype());
                        auto neg_sx = Tensor::make(UnaryOp{UnaryOpType::NEG}, {sx}, node->shape(), node->device_type(),
                                                   node->dtype());
                        auto g = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, neg_sx}, node->shape(),
                                              node->device_type(), node->dtype());
                        parent_grads = {g};
                        break;
                    }
                    case UnaryOpType::SIGMOID: {
                        ...
                        auto one = Tensor::make(ConstantOp{ConstantOpType::FULL, 1.0}, {}, node->shape(),
                                                node->device_type(), node->dtype());
                        auto one_minus =
                            Tensor::make(BinaryOp{BinaryOpType::SUB}, {one, utils::Ref<const Tensor>(node)},
                                         node->shape(), node->device_type(), node->dtype());
                        auto deriv =
                            Tensor::make(BinaryOp{BinaryOpType::MUL}, {utils::Ref<const Tensor>(node), one_minus},
                                         node->shape(), node->device_type(), node->dtype());
                        auto g = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, deriv}, node->shape(),
                                              node->device_type(), node->dtype());
                        parent_grads = {g};
                        break;
                    }
                    case UnaryOpType::SILU: {
                        ...
                        auto s =
                            Tensor::make(UnaryOp{UnaryOpType::SIGMOID}, {x}, x->shape(), x->device_type(), x->dtype());
                        auto one = Tensor::make(ConstantOp{ConstantOpType::FULL, 1.0}, {}, x->shape(), x->device_type(),
                                                x->dtype());
                        auto one_minus = Tensor::make(BinaryOp{BinaryOpType::SUB}, {one, s}, x->shape(),
                                                      x->device_type(), x->dtype());
                        auto x_term = Tensor::make(BinaryOp{BinaryOpType::MUL}, {x, one_minus}, x->shape(),
                                                   x->device_type(), x->dtype());
                        auto inner = Tensor::make(BinaryOp{BinaryOpType::ADD}, {one, x_term}, x->shape(),
                                                  x->device_type(), x->dtype());
                        auto deriv = Tensor::make(BinaryOp{BinaryOpType::MUL}, {s, inner}, x->shape(), x->device_type(),
                                                  x->dtype());
                        auto g = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, deriv}, node->shape(),
                                              node->device_type(), node->dtype());
                        parent_grads = {g};
                    ...
                    case BinaryOpType::SUB: {
                        ga = reduce_grad_to_parent(grad_this, a->shape());
                        auto neg_g = Tensor::make(UnaryOp{UnaryOpType::NEG}, {grad_this}, grad_this->shape(),
                                                  grad_this->device_type(), grad_this->dtype());
                        gb = reduce_grad_to_parent(neg_g, b->shape());
                        break;
                    }
                    case BinaryOpType::MUL: {
                        auto ga_raw = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, b}, node->shape(),
                                                   node->device_type(), node->dtype());
                        ga = reduce_grad_to_parent(ga_raw, a->shape());
                        auto gb_raw = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, a}, node->shape(),
                                                   node->device_type(), node->dtype());
                        gb = reduce_grad_to_parent(gb_raw, b->shape());
                        break;
                    }
                    case BinaryOpType::DIV: {
                        auto ga_raw = Tensor::make(BinaryOp{BinaryOpType::DIV}, {grad_this, b}, node->shape(),
                                                   node->device_type(), node->dtype());
                        ga = reduce_grad_to_parent(ga_raw, a->shape());
                        auto b2 =
                            Tensor::make(BinaryOp{BinaryOpType::MUL}, {b, b}, b->shape(), b->device_type(), b->dtype());
                        auto neg_g = Tensor::make(UnaryOp{UnaryOpType::NEG}, {grad_this}, grad_this->shape(),
                                                  grad_this->device_type(), grad_this->dtype());
                        auto num = Tensor::make(BinaryOp{BinaryOpType::MUL}, {neg_g, a}, node->shape(),
                                                node->device_type(), node->dtype());
                        auto gb_raw = Tensor::make(BinaryOp{BinaryOpType::DIV}, {num, b2}, node->shape(),
                                                   node->device_type(), node->dtype());
                        gb = reduce_grad_to_parent(gb_raw, b->shape());
                        break;
                    }
                    case BinaryOpType::POW: {
                        auto one = Tensor::make(ConstantOp{ConstantOpType::FULL, 1.0}, {}, b->shape(), b->device_type(),
                                                b->dtype());
                        auto b_minus_1 = Tensor::make(BinaryOp{BinaryOpType::SUB}, {b, one}, b->shape(),
                                                      b->device_type(), b->dtype());
                        auto a_pow_bm1 = Tensor::make(BinaryOp{BinaryOpType::POW}, {a, b_minus_1}, node->shape(),
                                                      a->device_type(), a->dtype());
                        auto term_a = Tensor::make(BinaryOp{BinaryOpType::MUL}, {b, a_pow_bm1}, node->shape(),
                                                   a->device_type(), a->dtype());
                        ...
                        auto ln_a =
                            Tensor::make(UnaryOp{UnaryOpType::LOG}, {a}, a->shape(), a->device_type(), a->dtype());
                        auto out_times_ln_a =
                            Tensor::make(BinaryOp{BinaryOpType::MUL}, {utils::Ref<const Tensor>(node), ln_a},
                                         node->shape(), node->device_type(), node->dtype());
                        auto gb_raw = Tensor::make(BinaryOp{BinaryOpType::MUL}, {grad_this, out_times_ln_a},
                                                   node->shape(), node->device_type(), node->dtype());
                        gb = reduce_grad_to_parent(gb_raw, b->shape());
                        break;
                    }
                    ...
                    case ReduceOpType::MAX: {
                        auto b_out = Tensor::make(MovementOp{MovementOpType::BROADCAST, x->shape()},
                                                  {utils::Ref<const Tensor>(node)},
                                                  common::AccessMeta::broadcast_from(node->access_meta(), x->shape()),
                                                  x->device_type(), x->dtype());
                        auto mask = Tensor::make(BinaryOp{BinaryOpType::CMP_EQ}, {x, b_out}, x->shape(),
                                                 x->device_type(), x->dtype());
                        auto b_g =
                            Tensor::make(MovementOp{MovementOpType::BROADCAST, x->shape()}, {grad_this},
                                         common::AccessMeta::broadcast_from(grad_this->access_meta(), x->shape()),
                                         grad_this->device_type(), grad_this->dtype());
                        auto g = Tensor::make(BinaryOp{BinaryOpType::MUL}, {b_g, mask}, x->shape(), x->device_type(),
                                              x->dtype());
                        parent_grads = {g};
                        break;
                    }
                    ...
                    auto At = Tensor::make(MovementOp{MovementOpType::PERMUTE, Xt_axes}, {A},
                                           ...
                                           A->device_type(), A->dtype());
                    auto dB = Tensor::make(MatMulOp{}, {At, grad_this}, {B->shape()[0], B->shape()[1]},
                                           B->device_type(), B->dtype());
                    parent_grads = {dA, dB};
        ...
        for (size_t i = 0; i < children.size(); ++i) {
            ...
            slot = slot ? Tensor::make(BinaryOp{BinaryOpType::ADD}, {slot, pgrad}, slot->shape(), slot->device_type(),
                                       slot->dtype())
                        : pgrad;
        }
    }
