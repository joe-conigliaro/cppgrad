...
struct GatherOp {}; // table[V, D, ...], indices[...] ->
// output[indices->shape(), D, ...] (gathers along axis 0)
struct GatherAxisOp {
    int axis;
}; // gather elements along arbitrary axis
struct ScatterOp {
    int axis;
}; // scatter values into base at indexed positions along axis
...
// In-place autoregressive cache append. Writes `values` into a preallocated
// cache buffer at [.., start : start+S, ..] along `axis` and returns a view of
// the cache covering
// [.., 0 : start+S, ..]. The write and the read-view are a single atomic node,
// so there is no read-after-write hazard on the (hazard-naive) executor, and
// the in-place write makes each decode step O(S) instead of the O(context) copy
// that ConcatOp incurs per step. Inference only (no backward); batch dim must
// be 1 (autoregressive decode).
struct CacheUpdateOp {
    int axis;
    size_t start;
};
...
// Fused flash attention (inference only, no backward). Computes softmax(scale *
// Q Kᵀ + causal_mask) V WITHOUT materializing the [S, KV] score matrix --
// online-softmax streaming over keys, O(1) extra memory. Grouped-query: query
// head h reads kv head h/n_rep (n_rep = nH/nKV). Causal masking is by position
// (no mask tensor): query row s (absolute position q_offset+s) attends keys [0,
// q_offset+s]. Inputs in native layout (no permute/copy): q [B,S,nH,Dh], k,v
// [B,KV,nKV,Dh]; output [B,S,nH,Dh].
...
// Fused RMSNorm over the last axis: out = x * rsqrt(mean(x^2)+eps) * weight, in
// one kernel pass (vs square->reduce->rsqrt->mul->mul). Inference-only
// (non-differentiable); training uses the composite. children = {x, weight}.
struct RMSNormOp {
    float eps;
};
...
// defines how many auxiliary metadata buffers it carries and what they mean
// (see aux_buffer_count and the QuantizedMatMulOp doc below); that decouples
// the op/backend ABI from any one scheme's metadata layout.
enum class QuantScheme {
    // MLX affine family: w = scale*q + bias, unsigned codes, per-group
    // scale+bias. aux = {scales,
    // biases}. The code WIDTH is a parameter (QuantParams::bits / pack_factor),
    // NOT part of the
    // scheme -- the dequant math and metadata layout are identical for 4/6/8-bit;
    // only the unpacking
    // (codes per storage word) differs. The backend kernel branches on
    // bits/pack_factor.
    MLX_AFFINE,
};

// Number of scheme-specific auxiliary metadata buffers a scheme supplies to
// quantized_matmul, in addition to the activation and packed qweight. Used to
// validate op inputs. (Width-independent.)
inline int aux_buffer_count(QuantScheme scheme) {
    switch (scheme) {
    case QuantScheme::MLX_AFFINE:
        return 2; // scales, biases
    }
    return 0;
}
...
struct QuantParams {
    ...
    int pack_factor = 4; // codes packed per storage word (32/bits; e.g. 4 u8 or 8
                         // u4 per uint32)
};

// Quantized matmul (inference only, no backward): out = A @ dequant(W)^T.
// `params` carries the quant scheme (+ bits/group_size/packing); the backend
// dispatches on scheme. Op inputs are [activation [M,K], packed qweight,
// aux...], where the trailing aux buffers are scheme-defined (MLX_AFFINE:
// scales then biases). The backend receives the aux buffers as an ordered list,
// so schemes with a different count/typing of metadata (k-quants' multi-level
// scales, AWQ qzeros, symmetric int with no bias) fit without changing the
// interface.
struct QuantizedMatMulOp {
    QuantParams params;
};
...
// NOTE: ordinal values are the op codes passed to the Metal unary kernel
// (apply_unary) -- keep in sync.
...
inline const char *to_string(const RandomOp &op) {
    switch (op.type) {
    case RandomOpType::UNIFORM:
        return "RandomOp:UNIFORM";
    case RandomOpType::NORMAL:
        return "RandomOp:NORMAL";
    }
}
...
inline const char *to_string(const BinaryOp &op) {
    ...
    case BinaryOpType::CMP_EQ:
        return "BinaryOp:CMP_EQ";
    case BinaryOpType::CMP_GT:
        return "BinaryOp:CMP_GT";
...
inline constexpr bool is_start_node_v =
    std::is_same_v<std::decay_t<T>, ConstantOp> || std::is_same_v<std::decay_t<T>, RandomOp> ||
...
inline constexpr bool is_differentiable_v =
    !std::is_same_v<std::decay_t<T>, ConstantOp> && !std::is_same_v<std::decay_t<T>, RandomOp> &&
    !std::is_same_v<std::decay_t<T>, LeafOp> && !std::is_same_v<std::decay_t<T>, AssignOp> &&
