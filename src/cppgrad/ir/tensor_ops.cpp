...
static utils::Ref<Tensor> binary(BinaryOpType op, const utils::Ref<const Tensor> &a,
                                 const utils::Ref<const Tensor> &b) {
...
utils::Ref<Tensor> assign(const utils::Ref<const Tensor> &dst, const utils::Ref<const Tensor> &src) {
    if (ir::GradMode::enabled)
        throw std::runtime_error("assign: forbidden in grad mode (AssignOp is a non-differentiable in-place update). "
                                 "Wrap optimizer updates in `ir::NoGradScope`.");
    if (!dst->is_canonical_leaf())
        throw std::runtime_error("assign: dst tensor is not a canonical leaf");
    if (dst->shape() != src->shape())
        throw std::runtime_error("assign: shape mismatch");
    if (dst->dtype() != src->dtype())
        throw std::runtime_error("assign: dtype mismatch");
    if (dst->device_type() != src->device_type())
        throw std::runtime_error("assign: device mismatch (use `src.to(dst->device_type())` first)");
...
utils::Ref<Tensor> cache_update(const utils::Ref<const Tensor> &cache, const utils::Ref<const Tensor> &values, int axis,
                                size_t start) {
    if (ir::GradMode::enabled)
        throw std::runtime_error("cache_update: forbidden in grad mode (in-place op, no backward).");
    if (!cache->is_canonical_leaf())
        throw std::runtime_error("cache_update: cache must be a canonical leaf");
    if (cache->shape().size() != values->shape().size())
        throw std::runtime_error("cache_update: rank mismatch");
    ...
    if (cache->dtype() != values->dtype()) {
        auto is_float = [](common::DType d) {
            return d == common::DType::FLOAT16 || d == common::DType::BFLOAT16 || d == common::DType::FLOAT32 ||
                   d == common::DType::FLOAT64;
        };
    ...
    if (cache->device_type() != values->device_type())
        throw std::runtime_error("cache_update: device mismatch");
    if (cache->shape()[0] != 1)
        throw std::runtime_error("cache_update: requires batch dim 1 (autoregressive decode)");
    const auto &cshape = cache->shape();
    size_t S = values->shape()[axis];
    size_t end = start + S;
    if (end > cshape[axis])
        throw std::runtime_error("cache_update: write past end of preallocated cache");
...
utils::Ref<Tensor> silu(const utils::Ref<const Tensor> &t) {
    return unary(UnaryOpType::SILU, t);
} // x*sigmoid(x), fused
utils::Ref<Tensor> sigmoid(const utils::Ref<const Tensor> &t) {
    return unary(UnaryOpType::SIGMOID, t);
} // 1/(1+e^-x), fused
...
utils::Ref<Tensor> sum(const utils::Ref<const Tensor> &t, const std::vector<int> &axes, bool keep_dims) {
    ...
    if (axes_in.empty()) {
        axes_in.resize(rank);
        std::iota(axes_in.begin(), axes_in.end(), 0);
    }
...
utils::Ref<Tensor> flash_attention(const utils::Ref<const Tensor> &q, const utils::Ref<const Tensor> &k,
    ...
    if (k->shape() != v->shape())
        throw std::runtime_error("flash_attention: k/v shape mismatch");
    if ((int)qs[2] != n_rep * (int)ks[2])
        throw std::runtime_error("flash_attention: nH != n_rep*nKV");
...
utils::Ref<Tensor> quantized_matmul(const utils::Ref<const Tensor> &a, const utils::Ref<const Tensor> &qweight,
    ...
    if (qweight->shape().size() != 2 || qweight->shape()[1] * static_cast<size_t>(params.pack_factor) != K)
    ...
    return Tensor::make(QuantizedMatMulOp{params}, inputs, out_shape, a->device_type(), od);
}
...
utils::Ref<Tensor> gather(const utils::Ref<const Tensor> &table, const utils::Ref<const Tensor> &indices) {
    ...
    auto out_dtype = (table->dtype() == common::DType::BFLOAT16) ? common::DType::FLOAT32 : table->dtype();
...
utils::Ref<Tensor> gather_axis(const utils::Ref<const Tensor> &tensor, const utils::Ref<const Tensor> &indices,
    ...
    if (ax < 0 || ax >= rank)
        throw std::runtime_error("gather_axis: axis out of range");
...
utils::Ref<Tensor> scatter_axis(const utils::Ref<const Tensor> &base, const utils::Ref<const Tensor> &values,
    ...
    int ax = axis < 0 ? rank + axis : axis;
    if (ax < 0 || ax >= rank)
        throw std::runtime_error("scatter_axis: axis out of range");
    size_t n = indices->numel();
    if (values->shape()[ax] != n)
        throw std::runtime_error("scatter_axis: values shape mismatch at axis");
...
utils::Ref<const Tensor> contiguous(const utils::Ref<const Tensor> &t) {
    ...
    if (am.contiguous && am.offset == 0)
        return t;
    return Tensor::make(CopyOp{}, {t}, common::AccessMeta::contiguous_from(t->shape(), 0), t->device_type(),
                        t->dtype());
}
...
utils::Ref<Tensor> reshape_view(const utils::Ref<const Tensor> &t, const std::vector<size_t> &new_shape) {
    if (utils::vector::numel(t->shape()) != utils::vector::numel(new_shape))
        throw std::runtime_error("reshape_view: numel must match");
...
utils::Ref<Tensor> permute(const utils::Ref<const Tensor> &t, const std::vector<size_t> &axes) {
    return Tensor::make(MovementOp{MovementOpType::PERMUTE, axes}, {t},
                        common::AccessMeta::permute_from(t->access_meta(), axes), t->device_type(), t->dtype());
}
...
utils::Ref<Tensor> broadcast(const utils::Ref<const Tensor> &t, const std::vector<size_t> &shape) {
    return Tensor::make(MovementOp{MovementOpType::BROADCAST, shape}, {t},
                        common::AccessMeta::broadcast_from(t->access_meta(), shape), t->device_type(), t->dtype());
}
...
utils::Ref<Tensor> slice(const utils::Ref<const Tensor> &t, const std::vector<size_t> &begin,
    ...
    return Tensor::make(MovementOp{MovementOpType::SLICE, steps, begin, end}, {t},
                        common::AccessMeta::slice_from(t->access_meta(), begin, end, steps), t->device_type(),
                        t->dtype());
}
...
utils::Ref<Tensor> reshape(const utils::Ref<const Tensor> &t, const std::vector<size_t> &new_shape) {
    ...
    if (t->access_meta().contiguous)
        return reshape_view(t, new_shape);
