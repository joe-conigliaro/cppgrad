...
parameter(const std::vector<size_t> &shape,
    ...
    if (allocate_now) {
        ...
        if (!device_obj)
            throw std::runtime_error("parameter: device not found");
    ...
    if (!param->is_canonical_leaf())
        throw std::runtime_error("parameter: non-canonical leaf");
...
inline utils::Ref<Tensor> parameterize(const utils::Ref<Tensor> &t) {
    if (!t)
        throw std::runtime_error("parameterize: null tensor");

    if (t->is_canonical_leaf()) {
        auto buf = t->eval();
        if (!buf)
            throw std::runtime_error("parameterize: canonical leaf without buffer");
    ...
    auto buf = t->eval();
    if (!buf)
        throw std::runtime_error("parameterize: realization failed (null buffer)");

    auto param = Tensor::make_leaf(buf, t->shape(), t->device_type(), t->dtype());
    param->set_requires_grad(true);

    if (!param->is_canonical_leaf())
        throw std::runtime_error("parameterize: non-canonical leaf");
...
inline utils::Ref<Tensor> commit(const utils::Ref<Tensor> &t) {
    if (!t)
        throw std::runtime_error("commit: null tensor");
    ...
    if (!buf)
        throw std::runtime_error("commit: realization failed (null buffer)");

    auto leaf = Tensor::make_leaf(buf, t->shape(), t->device_type(), t->dtype());
    leaf->set_requires_grad(false);
    if (!leaf->is_canonical_leaf())
        throw std::runtime_error("commit: non-canonical leaf");
    return leaf;
}

// Convenience alias
inline utils::Ref<ir::Tensor> trainable_from(const utils::Ref<ir::Tensor> &init) { return ir::parameterize(init); }
