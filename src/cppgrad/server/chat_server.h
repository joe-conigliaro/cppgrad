...
inline std::optional<Qwen3Config> parse_config(const std::string &name) {
    if (name == "0.6b")
        return Qwen3Config::get_0_6b();
    if (name == "1.5b")
        return Qwen3Config::get_1_5b();
    if (name == "3b")
        return Qwen3Config::get_3b();
    if (name == "4b")
        return Qwen3Config::get_4b();
    if (name == "7b")
        return Qwen3Config::get_7b();
    if (name == "27b")
...
inline size_t utf8_complete_len(const std::string &s) {
    ...
    if (i == 0)
        return n; // all continuation bytes: nothing better to do
...
inline bool parse_hermes_tool_calls(const std::string &text, std::vector<ToolCall> &calls, std::string &clean_text) {
...
inline ChatMessage to_chat_message(const Message &m) {
    ...
    for (const auto &tc : m.tool_calls) {
        ...
        try {
            ctc.arguments = nlohmann::json::parse(tc.function.arguments);
        } catch (...) {
            ctc.arguments = nlohmann::json::object();
        }
...
class ChatServer {
  public:
    bool load_model(const std::string &model_dir, const std::string &config_name = "27b", bool quantize = true) {
        ...
        stop_tokens_.clear();
        for (int id : {im_end_id_, eot_id_})
            if (id >= 0)
                stop_tokens_.push_back(id);
    ...
    void set_mtp(bool enabled, int n_draft = 4) {
        mtp_enabled_ = enabled;
        mtp_n_draft_ = n_draft;
    }
    ...
    bool load_draft_model(const std::string &dir, const std::string &config_name, int n_draft = 4,
                          bool quantize = true) {
    ...
    std::string decode_mode() const {
        if (draft_model_ && n_draft_ >= 2)
            return "speculative (draft model, n_draft=" + std::to_string(n_draft_) + ")";
    ...
    std::vector<int32_t> build_prompt(const std::vector<ChatMessage> &msgs, const std::vector<ChatTool> &tools) {
    ...
    std::vector<int32_t> build_prompt(const ChatCompletionRequest &req) {
        ...
        for (const auto &m : req.messages)
            msgs.push_back(to_chat_message(m));
        std::vector<ChatTool> tools;
        tools.reserve(req.tools.size());
        for (const auto &t : req.tools)
            tools.push_back(to_tool_def(t));
    ...
    GenText generate_text(const std::vector<int32_t> &prompt_ids, int max_tokens, const SamplingParams &sampling = {}) {
        ...
        return {tokenizer_->decode(ids), (int)prompt_ids.size(), (int)ids.size(), (int)ids.size() >= max_tokens};
    }
    ...
    GenStream stream_generate(const std::vector<int32_t> &prompt_ids, int max_tokens, const SamplingParams &sampling,
                              OnText on_text, OnTool on_tool) {
        std::lock_guard<std::mutex> lock(gen_mutex_);
        bool in_tool = false;
        ...
        auto cb = [&](int32_t id) -> bool {
            if (id == im_end_id_ || id == eot_id_)
                return true; // suppress stop tokens
            if (!in_tool && id == tool_call_start_) {
                in_tool = true;
                tool_ids.clear();
                return true;
            }
    ...
    static SamplingParams make_sampling(std::optional<double> temperature, std::optional<double> top_p,
                                        std::optional<int> top_k, std::optional<uint64_t> seed) {
        SamplingParams sp;
    ...
    void chat_complete_stream(const ChatCompletionRequest &req, std::function<bool(const std::string &)> callback) {
  ...
  private:
    ...
    std::vector<int32_t> decode(const std::vector<int32_t> &prompt_ids, int max_tokens, Qwen3Model::TokenCallback cb,
        ...
        if (draft_model_ && n_draft_ >= 2)
            return model_->generateSpeculative(prompt_ids, max_tokens, std::move(cb), stop_tokens_, *draft_model_,
                                               n_draft_, sampling, spec);
        if (mtp_enabled_ && model_->has_mtp() && mtp_n_draft_ >= 2)
            return model_->generateSpeculativeMTP(prompt_ids, max_tokens, std::move(cb), stop_tokens_, mtp_n_draft_,
                                                  sampling, /*accept_out=*/nullptr, spec);
        ...
        auto gen = nn::llm::generate_with_prefix_cache(
            *model_, session_, prompt_ids, max_tokens, [&](int32_t id) { return cb(id); }, stop, sampling);
        std::fprintf(stderr, "[prefix-cache] reused %zu / %zu prompt tokens (prefilled %zu)\n", session_.reused,
                     prompt_ids.size(), session_.prefilled);
