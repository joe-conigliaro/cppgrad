...
inline Request parse_request(const nlohmann::json& j) {
    ...
    if (j.contains("system")) {
        ...
        if (!sys.empty()) { ChatMessage m; m.role = "system"; m.content = sys; r.messages.push_back(std::move(m)); }
    }
    ...
    if (j.contains("messages") && j["messages"].is_array()) {
        for (const auto& m : j["messages"]) {
            ...
            for (auto& tr2 : tool_results) r.messages.push_back(std::move(tr2));
            if (!cm.content.empty() || !cm.tool_calls.empty()) r.messages.push_back(std::move(cm));
        }
    }
    return r;
}
...
inline nlohmann::json handle_messages(ChatServer& server, const Request& req) {
    ...
    if (!clean.empty()) content.push_back({{"type", "text"}, {"text", clean}});
    for (const auto& tc : calls) {
        nlohmann::json input;
        try { input = nlohmann::json::parse(tc.function.arguments); }
        catch (...) { input = nlohmann::json::object(); }
        content.push_back({{"type", "tool_use"}, {"id", tool_use_id()},
    ...
    return {
        ...
        {"model", server.model_name()}, {"content", content},
        {"stop_reason", stop_reason}, {"stop_sequence", nullptr},
...
inline void handle_messages_stream(ChatServer& server, const Request& req,
    ...
    event("message_start", {{"type", "message_start"}, {"message", {
        {"id", msg_id}, {"type", "message"}, {"role", "assistant"}, {"model", server.model_name()},
        {"content", nlohmann::json::array()}, {"stop_reason", nullptr}, {"stop_sequence", nullptr},
    ...
    auto open_text = [&]() {
        ...
        event("content_block_start", {{"type", "content_block_start"}, {"index", text_index},
    ...
    auto r = server.stream_generate(
        ...
        [&](const std::string& text) {
            open_text();
            event("content_block_delta", {{"type", "content_block_delta"}, {"index", text_index},
        ...
        [&](const ToolCall& tc) {
            ...
            event("content_block_start", {{"type", "content_block_start"}, {"index", idx},
                {"content_block", {{"type", "tool_use"}, {"id", tool_use_id()},
                                   {"name", tc.function.name}, {"input", nlohmann::json::object()}}}});
