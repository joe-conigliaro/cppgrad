    ...
    if (j.contains("function")) {
        ...
        if (fn.contains("arguments")) {
            tc.function.arguments =
                fn["arguments"].is_string() ? fn["arguments"].get<std::string>() : fn["arguments"].dump();
        }
    }
}
...
inline void to_json(nlohmann::json &j, const Choice &c) {
    ...
    if (!c.tool_calls.empty()) {
        ...
        for (const auto &tc : c.tool_calls)
            arr.push_back(tool_call_to_json(tc));
        msg["tool_calls"] = arr;
    }
...
inline void to_json(nlohmann::json &j, const ChatCompletionChunk &c) {
    j = {{"id", c.id}, {"object", c.object}, {"created", c.created}, {"model", c.model}, {"choices", c.choices}};
}
...
inline void to_json(nlohmann::json &j, const ModelList &m) { j = {{"object", m.object}, {"data", m.data}}; }
