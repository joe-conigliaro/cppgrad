...
static inline uint16_t float32_to_bf16(float v) {
    ...
    if ((x & 0x7fffffffu) > 0x7f800000u)
        return (uint16_t)((x >> 16) | 0x40u); // keep NaN
    x += 0x7fffu + ((x >> 16) & 1u);          // round-to-nearest-even
    return (uint16_t)(x >> 16);
}

// Convert raw safetensors data to FP32 buffer, handling BF16/F16/F64/F32/INT32.
static std::vector<float> safetensors_to_float32(const std::vector<uint8_t> &raw, common::DType dtype, size_t numel) {
...
static common::DType safetensors_dtype(const std::string &dt) {
    ...
    if (dt == "F64")
        return common::DType::FLOAT64;
    if (dt == "F16")
        return common::DType::FLOAT16;
    if (dt == "BF16")
        return common::DType::BFLOAT16;
    if (dt == "I32")
        return common::DType::INT32;
    if (dt == "I64")
        return common::DType::INT64;
    if (dt == "I8")
        return common::DType::INT8;
    if (dt == "U8")
        return common::DType::UINT8;
    if (dt == "U32")
        return common::DType::UINT32;
    if (dt == "BOOL")
        return common::DType::BOOL8;
...
static std::pair<uint64_t, nlohmann::json> read_safetensors_header(std::ifstream &fs) {
    ...
    auto try_header_size = [&](uint64_t hs) -> bool {
        if (hs == 0 || hs > static_cast<uint64_t>(file_size) / 10)
            return false;
        fs.seekg(8);
    ...
    if (!fs)
        throw std::runtime_error("safetensors: failed to read header");
...
inline utils::Ref<ir::Tensor> dequant_mlx_affine(const utils::Ref<ir::Tensor> &weight_u32,
                                                 const utils::Ref<ir::Tensor> &scales_raw,
                                                 const utils::Ref<ir::Tensor> &biases_raw,
                                                 backend::DeviceType device_type, int group_size = 64) {
    auto device = backend::DeviceManager::device(device_type);
    if (!device)
        throw std::runtime_error("dequant: device not found");

    // Get weight data (U32 packed as int8 bytes)
    auto weight_buf = weight_u32->eval();
    if (!weight_buf)
        throw std::runtime_error("dequant: weight eval failed");
...
inline std::map<std::string, utils::Ref<ir::Tensor>>
dequantize_mlx_tensors(std::map<std::string, utils::Ref<ir::Tensor>> all_tensors, backend::DeviceType device_type);
...
load_safetensors(const std::string &path,
    ...
    auto *device = backend::DeviceManager::device(device_type);
    if (!device)
        throw std::runtime_error("safetensors: device not found");
    ...
    for (auto &[name, info] : header.items()) {
        ...
        if (!fs)
            throw std::runtime_error("safetensors: failed to read tensor '" + name + "'");
...
inline std::map<std::string, utils::Ref<ir::Tensor>>
load_safetensors(const std::vector<std::string> &paths,
                 backend::DeviceType device_type = backend::DeviceManager::default_device_type()) {
...
dequantize_mlx_tensors(std::map<std::string, utils::Ref<ir::Tensor>> all_tensors, backend::DeviceType device_type) {
    ...
    for (auto &[name, tensor] : all_tensors) {
        ...
        if (string_ends_with(name, ".scales") || string_ends_with(name, ".biases")) {
            ...
            if (is_quant_base)
                continue; // skip scales/biases for quantized weights
        }
