  ...
  public:
    explicit BPETokenizer(const std::string &tokenizer_json_path) {
        ...
        for (auto &[tok, id] : j["model"]["vocab"].items()) {
            int i = id.get<int>();
            vocab_[tok] = i;
            if ((int)id_to_tok_.size() <= i)
                id_to_tok_.resize(i + 1);
            id_to_tok_[i] = tok;
        }
        ...
        if (j.contains("added_tokens")) {
            for (auto &t : j["added_tokens"]) {
                ...
                special_[content] = id;
                if ((int)id_to_tok_.size() <= id)
                    id_to_tok_.resize(id + 1);
    ...
    std::vector<int32_t> encode(const std::string &text) const {
        ...
        size_t i = 0;
        while (i < text.size()) {
            size_t next = std::string::npos, next_len = 0;
            int next_id = -1;
            for (auto &[content, id] : special_) {
                ...
                if (pos != std::string::npos && (pos < next || (pos == next && content.size() > next_len))) {
                    next = pos;
                    next_len = content.size();
                    next_id = id;
                }
            }
            size_t gap_end = (next == std::string::npos) ? text.size() : next;
            if (gap_end > i)
                encode_chunk(text.substr(i, gap_end - i), ids);
    ...
    std::string decode(const std::vector<int32_t> &ids, bool skip_special = false) const {
        std::string out;
        for (int32_t id : ids) {
            if (id < 0 || id >= (int)id_to_tok_.size())
                continue;
            if (special_is_special_.count(id)) {
                if (!skip_special)
                    out += id_to_tok_[id]; // special content is literal
                continue;
            }
  ...
  private:
    ...
    void compile_regex(const nlohmann::json &pre) {
        ...
        std::function<void(const nlohmann::json &)> find = [&](const nlohmann::json &p) {
            ...
            if (p.contains("pretokenizers"))
                for (auto &s : p["pretokenizers"])
                    find(s);
        };
        find(pre);
        if (pattern.empty())
            throw std::runtime_error("tokenizer: no Split regex in pre_tokenizer");

        int errcode;
        PCRE2_SIZE erroff;
        re_ = pcre2_compile((PCRE2_SPTR)pattern.c_str(), pattern.size(), PCRE2_UTF | PCRE2_UCP, &errcode, &erroff,
                            nullptr);
        if (!re_) {
            PCRE2_UCHAR buf[256];
    ...
    static uint32_t utf8_next(const std::string &s, size_t &p) {
        ...
        if ((c >> 5) == 0x6) {
            uint32_t cp = ((c & 0x1F) << 6) | (s[p + 1] & 0x3F);
            p += 2;
            return cp;
        }
        if ((c >> 4) == 0xE) {
            uint32_t cp = ((c & 0x0F) << 12) | ((s[p + 1] & 0x3F) << 6) | (s[p + 2] & 0x3F);
            p += 3;
            return cp;
        }
        uint32_t cp = ((c & 0x07) << 18) | ((s[p + 1] & 0x3F) << 12) | ((s[p + 2] & 0x3F) << 6) | (s[p + 3] & 0x3F);
        p += 4;
        return cp;
    }
};
