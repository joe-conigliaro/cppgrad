...
class RefCounted {
  public:
    ...
    void inc_ref() const noexcept { _ref_count.fetch_add(1, std::memory_order_relaxed); }
    ...
    long use_count() const noexcept { return _ref_count.load(std::memory_order_relaxed); }
...
template <typename T, typename U> inline bool operator==(const Ref<T> &lhs, const Ref<U> &rhs) noexcept {
...
template <typename T> inline bool operator==(const Ref<T> &lhs, std::nullptr_t) noexcept {
    return lhs.get() == nullptr;
}

template <typename T> inline bool operator==(std::nullptr_t, const Ref<T> &rhs) noexcept {
...
template <typename T> struct hash<cppgrad::utils::Ref<T>> {
