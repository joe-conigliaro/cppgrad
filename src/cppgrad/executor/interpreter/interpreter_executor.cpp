...
void InterpreterExecutor::realize_scheduled(const std::vector<DeviceSchedule> &schedules) {
    ...
    for (const auto &ds : schedules) {
        ...
        if (!out_device)
            throw std::runtime_error("InterpreterExecutor: device not found");
        ...
        for (const auto &t : ds.schedule) {
            ...
            if (std::holds_alternative<cppgrad::ir::LeafOp>(t->op())) {
                if (!t->realized_buffer() && t->numel() > 0) {
                    throw std::runtime_error("InterpreterExecutor: leaf tensor has no storage. Did you create a "
                                             "parameter with allocate_now=false and forget to initialize it?");
                }
                if (t->schedule())
                    realized[t] = t->schedule();
                continue;
            }
            ...
            std::visit(
                [&](auto &&op) {
                    ...
                    if constexpr (std::is_same_v<T, cppgrad::ir::ConstantOp>) {
                        if (op.type == cppgrad::ir::ConstantOpType::SCALAR &&
                            (t->numel() != 1 || !t->shape().empty())) {
                            throw std::runtime_error(
                                "InterpreterExecutor: ConstantOp of type SCALAR must be rank-0 scalar");
                        }
                        ...
                        if (!cppgrad::ir::GradMode::enabled) {
                            auto key = std::make_tuple((int)t->device_type(), (int)t->dtype(), t->numel(), op.value);
                        ...
                        auto vo = backend::View::from(t->access_meta());
                        if (!vo.is_identity()) {
                            // Catch here instead of just creating an identity view, it should be identity from
                            // upstream.
                            throw std::runtime_error("InterpreterExecutor: ReduceOp output must be identity "
                                                     "(contiguous / row-major dense, with offset=0)");
                        }
                        ...
                        const size_t B = q_t->shape()[0], S = q_t->shape()[1], nH = q_t->shape()[2],
                                     Dh = q_t->shape()[3];
                        const size_t KV = k_t->shape()[1], nKV = k_t->shape()[2];
                        out_device->backend()->flash_attention(*parents[0], *parents[1], *parents[2], *out_buf, B, S,
                                                               nH, Dh, KV, nKV, op.scale, op.n_rep, op.causal,
                                                               op.q_offset);
                        ...
                        for (size_t pi = 2; pi < parents.size(); ++pi)
                            aux.push_back(parents[pi].get());
                        out_device->backend()->quantized_matmul(*parents[0], *parents[1], aux, *out_buf, M, N, K,
                                                                op.params);
                        ...
                        auto write_am = cppgrad::common::AccessMeta::slice_from(
                            cache_t->access_meta(), begin, end, std::vector<size_t>(cache_t->shape().size(), 1));
                        ...
                        size_t D = 1;
                        for (size_t i = 1; i < table_tensor->shape().size(); ++i)
                            D *= table_tensor->shape()[i];
                        ...
                        auto vo = backend::View::from(common::AccessMeta::contiguous_from(t->shape(), 0));
                        out_device->backend()->scatter_axis_op(*parents[0], bv, *parents[1], vv, *parents[2], *out_buf,
                                                               vo, op.axis);
            ...
            // }
            if (!out_buf)
                throw std::runtime_error("InterpreterExecutor: failed to realize tensor");
            ...
            if (profile_on) {
                ...
                if (!std::holds_alternative<cppgrad::ir::MovementOp>(t->op())) {
                    ...
                    for (const auto &c : t->children())
                        if (c)
                            bytes += (uint64_t)c->numel() * common::size(c->dtype());
                }
