...
void Executor::build_schedule_dfs(const utils::Ref<const ir::Tensor> &t,
...
Executor::build_device_schedules(const std::vector<utils::Ref<const ir::Tensor>> &outs) {
    std::unordered_map<backend::DeviceType, std::vector<utils::Ref<const ir::Tensor>>> outs_by_dev;
    for (const auto &o : outs)
        if (o)
            outs_by_dev[o->device_type()].push_back(o);
    ...
    for (auto &[dev, dev_outs] : outs_by_dev) {
        ...
        for (const auto &out : dev_outs)
            build_schedule_dfs(out, visited, ord);
