...
class Executor {
  ...
  protected:
    ...
    static void build_schedule_dfs(const utils::Ref<const ir::Tensor> &t,
                                   std::unordered_set<utils::Ref<const ir::Tensor>> &visited,
                                   std::vector<utils::Ref<const ir::Tensor>> &order);

    static std::vector<DeviceSchedule> build_device_schedules(const std::vector<utils::Ref<const ir::Tensor>> &outs);
};
