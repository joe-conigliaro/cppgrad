...
inline float quant_gemv_word(uint w, device const uint32_t *wrow, device const float4 *A4, device const float *srow,
                             device const float *brow, uint wpg) {
    uint word = wrow[w];
    uint g = w / wpg;
    float s = srow[g], b = brow[g];
    float4 a = A4[w];
    float4 q = float4(float(word & 0xFFu), float((word >> 8) & 0xFFu), float((word >> 16) & 0xFFu),
                      float((word >> 24) & 0xFFu));
...
static inline void matmul_quant_gemm_tiled_impl(device const AT *A, device const uint32_t *QW, device const float *S,
                                                device const float *Bb, device OUTT *OUT, uint M, uint N, uint K,
                                                uint GS, threadgroup float (&As)[QT_BM][QT_BK],
                                                threadgroup float (&Ws)[QT_BK][QT_BN], uint3 tid, uint3 bid) {
    ...
    for (uint t = 0; t < ktiles; ++t) {
        ...
        for (uint k = 0; k < QT_BK; ++k) {
            ...
            for (uint i = 0; i < QT_RM; ++i)
                a[i] = As[tid.y * QT_RM + i][k];
            for (uint j = 0; j < QT_RN; ++j)
                w[j] = Ws[k][tid.x * QT_RN + j];
            for (uint i = 0; i < QT_RM; ++i)
                for (uint j = 0; j < QT_RN; ++j)
                    acc[i][j] += a[i] * w[j];
        }
    ...
    for (uint i = 0; i < QT_RM; ++i) {
        ...
        for (uint j = 0; j < QT_RN; ++j) {
            ...
            if (n < N)
                OUT[(ulong)m * N + n] = (OUTT)acc[i][j];
        }
    }
}

// fp32 A -> fp32 out (default prefill GEMM).
kernel void matmul_quant_gemm_tiled_f32(device const float *A [[buffer(0)]], device const uint32_t *QW [[buffer(1)]],
                                        device const float *S [[buffer(2)]], device const float *Bb [[buffer(3)]],
                                        device float *OUT [[buffer(4)]], constant uint32_t &M [[buffer(5)]],
                                        constant uint32_t &N [[buffer(6)]], constant uint32_t &K [[buffer(7)]],
...
kernel void matmul_quant_gemm_tiled_f32a_bf16o(
    device const float *A [[buffer(0)]], device const uint32_t *QW [[buffer(1)]], device const float *S [[buffer(2)]],
    device const float *Bb [[buffer(3)]], device bfloat *OUT [[buffer(4)]], constant uint32_t &M [[buffer(5)]],
    constant uint32_t &N [[buffer(6)]], constant uint32_t &K [[buffer(7)]], constant uint32_t &GS [[buffer(8)]],
...
kernel void matmul_quant_gemm_tiled_bf16a_f32o(
    device const bfloat *A [[buffer(0)]], device const uint32_t *QW [[buffer(1)]], device const float *S [[buffer(2)]],
    device const float *Bb [[buffer(3)]], device float *OUT [[buffer(4)]], constant uint32_t &M [[buffer(5)]],
    constant uint32_t &N [[buffer(6)]], constant uint32_t &K [[buffer(7)]], constant uint32_t &GS [[buffer(8)]],
    uint3 tid [[thread_position_in_threadgroup]], uint3 bid [[threadgroup_position_in_grid]]) {
    threadgroup float As[QT_BM][QT_BK];
    threadgroup float Ws[QT_BK][QT_BN];
...
kernel void matmul_quant_gemv_f32(device const float *A [[buffer(0)]],     // [1, K]
                                  ...
                                  constant uint32_t &N [[buffer(5)]], constant uint32_t &K [[buffer(6)]],
                                  constant uint32_t &GS [[buffer(7)]], uint n [[threadgroup_position_in_grid]],
