...
static void check_alloc(id<MTLBuffer> buf, size_t bytes, id<MTLDevice> dev) {
    ...
    throw std::runtime_error(
        "Metal buffer allocation failed: requested " + std::to_string(bytes) +
...
std::shared_ptr<Buffer> MetalAllocator::allocate(size_t num_elements, common::DType dtype) {
...
std::shared_ptr<Buffer> MetalAllocator::allocate(const void *src, size_t num_elements, common::DType dtype) {
    const size_t bytes = num_elements * size(dtype);
    if (bytes == 0) {
        return std::make_shared<Buffer>(nullptr, 0, dtype, DeviceType::METAL, this);
    }
...
void MetalAllocator::copy_device_to_host(void *host_dst, const Buffer &device_src) const {
    if (device_src.size_bytes() == 0 || !host_dst)
        return;
...
void MetalAllocator::copy_host_to_device(Buffer &device_dst, const void *host_src) const {
    if (device_dst.size_bytes() == 0 || !host_src)
        return;
