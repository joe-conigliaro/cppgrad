...
MetalExecutionContext::MetalExecutionContext(void *native_device, void *native_queue)
    : _device((__bridge id<MTLDevice>)native_device), _queue((__bridge id<MTLCommandQueue>)native_queue) {
...
void MetalExecutionContext::submit_compute(ComputeWork work) {
    ...
    if (kMaxKernels && _computeWork.size() >= kMaxKernels)
        flush();
}

void MetalExecutionContext::flush() {
    if (!_computeWork.empty()) {
        if (getenv("CPPGRAD_METAL_DISPATCH"))
            fprintf(stderr, "[metal] flush: %zu kernels\n", _computeWork.size());
        ...
        if (cap && atoi(cap) == s_fi) {
            ...
            if ([capMgr startCaptureWithDescriptor:d error:&err]) {
                ...
                fprintf(stderr, "[metal] capture start failed: %s\n", err.localizedDescription.UTF8String);
                capMgr = nil;
            }
        }
        ...
        if (_commandBuffer.status != MTLCommandBufferStatusCompleted) {
            ...
            fprintf(stderr,
                    "[metal] command buffer did not complete: status=%ld error=%s (domain=%s code=%ld) [%zu kernels]\n",
                    (long)_commandBuffer.status, cbErr ? cbErr.localizedDescription.UTF8String : "(none)",
                    cbErr ? cbErr.domain.UTF8String : "(none)", cbErr ? (long)cbErr.code : 0L, _computeWork.size());
        }

        if (capMgr) {
            [capMgr stopCapture];
            fprintf(stderr, "[metal] capture done\n");
        }
