...
struct View {
    ...
    static View from(const common::AccessMeta &acc) {
        ...
        if (v.rank > kMaxRank)
            throw std::runtime_error("View::from: rank exceeds kMaxRank");
        ...
        if (is_row_major_strides != acc.contiguous) {
            ...
            for (auto d : acc.shape)
                msg += std::to_string(d) + " ";
            msg += "} strides={";
            for (auto d : acc.strides)
                msg += std::to_string(d) + " ";
            msg +=
                "} contiguous=" + std::to_string(acc.contiguous) + ", expected=" + std::to_string(is_row_major_strides);
        ...
        if (v.rank == 2) {
            ...
            if (s1 == 1 && s0 == N)
                v.flags |= VIEW2D_ROWMAJ_NN;
            else if (s0 == 1 && s1 == M)
                v.flags |= VIEW2D_ROWMAJ_TN;
        }
        return v;
    }
};
