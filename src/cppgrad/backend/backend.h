...
class Backend {
  public:
    ...
    // Data Ops
    virtual void copy(Buffer &dst, const Buffer &src) const {
        cppgrad::backend::copy(dst, src);
    } // Use backend copy util by default
    virtual void fill(Buffer &buf, double value) const = 0;

    // Main Compute Ops
    virtual void unary_op(ir::UnaryOpType op_type, const Buffer &a, const backend::View &va, Buffer &out,
                          const backend::View &vo) const = 0;
    virtual void binary_op(ir::BinaryOpType op_type, const Buffer &a, const backend::View &va, const Buffer &b,
                           const backend::View &vb, Buffer &out, const backend::View &vo) const = 0;
    virtual void reduce_op(ir::ReduceOpType op_type, const Buffer &a, const backend::View &va, Buffer &out,
                           const backend::View &vo, const std::vector<int> &axes, bool keep_dims) const = 0;
    virtual void matmul(const Buffer &a, const backend::View &va, const Buffer &b, const backend::View &vb, Buffer &out,
                        const backend::View &vo) const = 0;
    ...
    virtual void flash_attention(const Buffer & /*q*/, const Buffer & /*k*/, const Buffer & /*v*/, Buffer & /*out*/,
                                 size_t /*B*/, size_t /*S*/, size_t /*nH*/, size_t /*Dh*/, size_t /*KV*/,
                                 size_t /*nKV*/, float /*scale*/, int /*n_rep*/, bool /*causal*/,
    ...
    virtual void rms_norm(const Buffer & /*x*/, const Buffer & /*weight*/, Buffer & /*out*/, size_t /*rows*/,
                          size_t /*D*/, float /*eps*/) const {
    ...
    virtual void quantized_matmul(const Buffer &a, const Buffer &qweight, const std::vector<const Buffer *> &aux,
                                  Buffer &out, size_t M, size_t N, size_t K, const ir::QuantParams &params) const = 0;
    ...
    virtual void gather_axis_op(const Buffer &tensor, const backend::View &tv, const Buffer &indices, Buffer &out,
                                const backend::View &ov, int axis) const = 0;

    // Scatter along axis: base + values at indices -> out (same shape as base)
    virtual void scatter_axis_op(const Buffer &base, const backend::View &bv, const Buffer &values,
