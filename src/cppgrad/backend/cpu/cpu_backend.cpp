...
void CPUBackend::rand_normal(Buffer &out, float mean, float stddev) const {
    cpu::dispatch_dtype(out.dtype(), make_templated([&](auto tag) {
                            ...
                            for (uint32_t i = 0; i < n; i += 2) {
                                ...
                                if (i + 1 < n)
                                    data[i + 1] = static_cast<T>(mean + stddev * z1);
                            }
                        }));
}
...
void CPUBackend::unary_op(ir::UnaryOpType op_type, const Buffer &a, const backend::View &va, Buffer &out,
                          const backend::View &vo) const {
    cpu::dispatch_dtype(out.dtype(), make_templated([&](auto tag) {
                            using T = typename decltype(tag)::type;
                            auto fn = [&](T x) -> T {
                                switch (op_type) {
                                case ir::UnaryOpType::RELU:
                                    return x > T(0) ? x : T(0);
                                case ir::UnaryOpType::EXP:
                                    return static_cast<T>(std::exp(static_cast<double>(x)));
                                case ir::UnaryOpType::LOG:
                                    return static_cast<T>(std::log(static_cast<double>(x)));
                                case ir::UnaryOpType::NEG:
                                    return -x;
                                case ir::UnaryOpType::TANH:
                                    return static_cast<T>(std::tanh(static_cast<double>(x)));
                                case ir::UnaryOpType::SIN:
                                    return static_cast<T>(std::sin(static_cast<double>(x)));
                                case ir::UnaryOpType::COS:
                                    return static_cast<T>(std::cos(static_cast<double>(x)));
                                case ir::UnaryOpType::SILU: {
                                    double s = 1.0 / (1.0 + std::exp(-static_cast<double>(x)));
                                    return static_cast<T>(static_cast<double>(x) * s);
                                }
                                case ir::UnaryOpType::SIGMOID:
...
void CPUBackend::binary_op(ir::BinaryOpType op_type, const Buffer &a, const backend::View &va, const Buffer &b,
    ...
    cpu::dispatch_dtype(out.dtype(), make_templated([&](auto tag) {
                            ...
                            auto fn = [&](T x, T y) -> T {
                                ...
                                case ir::BinaryOpType::DIV:
                                    return x / y;
                                case ir::BinaryOpType::POW:
                                    return static_cast<T>(std::pow(static_cast<double>(x), static_cast<double>(y)));
                                case ir::BinaryOpType::CMP_EQ:
                                    return T(x == y);
                                case ir::BinaryOpType::CMP_GT:
                                    return T(x > y);
                                case ir::BinaryOpType::MIN:
                                    return std::min(x, y);
                                case ir::BinaryOpType::MAX:
                                    return std::max(x, y);
                                }
                                return T(0);
                            };
                            if (va.is_identity() && vb.is_identity() && vo.is_identity() && same_shape(va, vb) &&
                                same_shape(va, vo)) {
                                // build std::vector<size_t> once for row-major flat helpers
                                std::vector<size_t> shape;
                                shape.reserve(va.rank);
                                for (uint32_t i = 0; i < va.rank; ++i)
                                    shape.push_back(va.shape[i]);
...
void CPUBackend::reduce_op(ir::ReduceOpType op_type, const Buffer &a, const backend::View &va, Buffer &out,
...
void CPUBackend::quantized_matmul(const Buffer &a, const Buffer &qweight, const std::vector<const Buffer *> &aux,
    ...
    case ir::QuantScheme::MLX_AFFINE: {
        ...
        if (params.bits != 8)
            throw std::runtime_error(
                "CPUBackend::quantized_matmul: MLX_AFFINE only supports bits=8 (4-bit kernel is TODO)");
        const Buffer &scales = *aux[0];
        const Buffer &biases = *aux[1];
        cpu::matmul_quant_affine_f32(
            static_cast<const float *>(a.data()), static_cast<const uint32_t *>(qweight.data()),
            static_cast<const float *>(scales.data()), static_cast<const float *>(biases.data()),
...
void CPUBackend::flash_attention(const Buffer &q, const Buffer &k, const Buffer &v, Buffer &out, size_t B, size_t S,
...
void CPUBackend::matmul(const Buffer &a, const backend::View &va, const Buffer &b, const backend::View &vb, Buffer &out,
                        const backend::View &vo) const {
    ...
    const bool b_bf16 = (b.dtype() == common::DType::BFLOAT16 && out.dtype() == common::DType::FLOAT32);
    ...
    auto build_subviews = [&](size_t bi, backend::View &va2, backend::View &vb2, backend::View &vo2) {
        ...
        va2.rank = 2;
        va2.shape[0] = (uint32_t)M;
        va2.shape[1] = (uint32_t)K;
        va2.strides[0] = va.strides[rank - 2];
        va2.strides[1] = va.strides[rank - 1];
        va2.offset = (uint32_t)a_off;
        va2.flags = 0;
        va2.numel = M * K;
        vb2.rank = 2;
        vb2.shape[0] = (uint32_t)K;
        vb2.shape[1] = (uint32_t)N;
        vb2.strides[0] = vb.strides[rank - 2];
        vb2.strides[1] = vb.strides[rank - 1];
        vb2.offset = (uint32_t)b_off;
        vb2.flags = 0;
        vb2.numel = K * N;
        vo2.rank = 2;
        vo2.shape[0] = (uint32_t)M;
        vo2.shape[1] = (uint32_t)N;
        vo2.strides[0] = vo.strides[rank - 2];
        vo2.strides[1] = vo.strides[rank - 1];
        vo2.offset = (uint32_t)o_off;
        vo2.flags = 0;
        vo2.numel = M * N;
    };
...
void CPUBackend::rms_norm(const Buffer &x, const Buffer &weight, Buffer &out, size_t rows, size_t D, float eps) const {
        ...
        cpu::parallel_for((size_t)0, N, [&](size_t s, size_t e) {
            for (size_t i = s; i < e; ++i) {
                ...
                if (k < 0 || static_cast<size_t>(k) >= V) {
                    ...
                    for (size_t j = 0; j < D; ++j)
                        o_ptr[i * D + j] = cpu::bf16_to_f32(t_ptr[k * D + j]);
                }
            }
        });
        return;
...
void CPUBackend::gather_axis_op(const Buffer &tensor, const backend::View &tv, const Buffer &indices, Buffer &out,
...
void CPUBackend::scatter_axis_op(const Buffer &base, const backend::View &bv, const Buffer &values,
