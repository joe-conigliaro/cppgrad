...
static std::vector<std::string> gather_shards(const std::string &path) {
    ...
    if (out.empty())
        out.push_back(path); // let the loader report a clear error
    return out;
}
...
static Args parse_args(int argc, char **argv) {
    Args args;
    static struct option long_opts[] = {{"model", required_argument, 0, 'm'},
    ...
    while ((c = getopt_long(argc, argv, "m:p:n:c:t:qh", long_opts, nullptr)) != -1) {
        ...
        case 'p':
            ...
            break;
        case 'n':
            args.max_tokens = std::stoi(optarg);
            break;
        case 'c':
...
int main(int argc, char **argv) {
    ...
    if (args.config_name == "0.6b")
        config = cppgrad::nn::llm::qwen::Qwen3Config::get_0_6b();
    else if (args.config_name == "1.5b")
        config = cppgrad::nn::llm::qwen::Qwen3Config::get_1_5b();
    else if (args.config_name == "3b")
        config = cppgrad::nn::llm::qwen::Qwen3Config::get_3b();
    else if (args.config_name == "4b")
        config = cppgrad::nn::llm::qwen::Qwen3Config::get_4b();
    else if (args.config_name == "7b")
        config = cppgrad::nn::llm::qwen::Qwen3Config::get_7b();
    else if (args.config_name == "27b")
        config = cppgrad::nn::llm::qwen::Qwen3Config::get_27b();
    else {
    ...
    std::cout << "[" << (is_qwen3_5 ? "Qwen3.5/3.6" : "Qwen3")
              << "] Device: " << cppgrad::backend::to_string(device_type) << "\n";
    ...
    std::string tok_path = !args.tokenizer_path.empty() ? args.tokenizer_path : derive_tokenizer_path(args.model_path);
    std::unique_ptr<cppgrad::io::BPETokenizer> tok;
    {
        std::ifstream tf(tok_path);
        if (tf.good()) {
            try {
                tok.reset(new cppgrad::io::BPETokenizer(tok_path));
            } catch (const std::exception &e) {
                std::cerr << "[Qwen3] tokenizer load failed: " << e.what() << "\n";
            }
        }
    }

    std::vector<int32_t> input_ids =
        tok ? tok->encode(args.prompt) : simple_tokenize(args.prompt, config.vocab_size, bos_token);
