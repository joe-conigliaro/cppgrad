...
static ServerArgs parse_args(int argc, char** argv) {
    ...
    while ((c = getopt_long(argc, argv, "m:c:p:h:q?", opts, nullptr)) != -1) {
        switch (c) {
            case 'm': a.model_dir = optarg; break;
            case 'c': a.config = optarg; break;
            case 'p': a.port = std::stoi(optarg); break;
            case 'h': a.host = optarg; break;
            case 'q': a.quant = true; break;
            case OPT_DRAFT_MODEL:  a.draft_dir = optarg; break;
            case OPT_DRAFT_CONFIG: a.draft_config = optarg; break;
...
int main(int argc, char** argv) {
    ...
    if (!chat.load_model(args.model_dir, args.config, args.quant)) {
    ...
    svr.Post("/v1/chat/completions", [&chat](const httplib::Request& req, httplib::Response& res) {
        ...
        if (std::getenv("CPPGRAD_DUMP_ONLY")) return send_error(res, 503, "dump-only mode (request captured)");
        ...
        if (request.messages.empty()) return send_error(res, 400, "messages must not be empty");
        ...
        if (request.stream.value_or(false)) {
            ...
            res.set_chunked_content_provider(
                "text/event-stream",
                [&chat, request](size_t /*offset*/, httplib::DataSink& sink) {
